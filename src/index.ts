import { core, fs, https, io, os, path, shell, TMP_DIR } from "./const";
...
async function get_slim() {
  ...
  try {
    ...
    throw new Error(
      "ERROR! Could not retrieve the current Slim version number.",
    );
  }

  URL = `https://github.com/slimtoolkit/slim/releases/${VER}`;
...
async function run() {
  ...
  if (
    process.env["DSLIM_HTTP_PROBE_OFF"] == "true" ||
    process.env["DSLIM_HTTP_PROBE"] == "false"
  ) {
  ...
  await shell.exec("docker tag", [
    report.minified_image,
    `${image}:${inputTag}`,
  ]);
